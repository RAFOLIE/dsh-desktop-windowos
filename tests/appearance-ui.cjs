...
(async () => {
 ...
 await page.addInitScript(() => {
   ...
   window.testCalls=[];
   window.__DSH_INITIAL_THEME__="dark";
   Object.defineProperty(navigator,'clipboard',{value:{readText:async()=>{if(window.clipboardReadFail)throw new Error('clipboard');return window.copiedTheme || '';},writeText:async text=>{if(window.clipboardFail)throw new Error('clipboard');window.copiedTheme=text;}}});
   ...
   window.__TAURI_INTERNALS__={
     ...
     async invoke(cmd,args={}){
       ...
       if(cmd==='window_shell_ready') {
         window.firstShellPaint={theme:document.documentElement.dataset.theme,bg:getComputedStyle(document.documentElement).getPropertyValue('--ep-bg').trim(),bar:!!document.querySelector('.titlebar')};
         return;
       }
 ...
 await page.waitForFunction(()=>window.firstShellPaint);
 assert.deepEqual(await page.evaluate(()=>window.firstShellPaint),{theme:'dark',bg:'#191919',bar:true});
 ...
 const lightPreset=page.getByRole('combobox',{name:'浅色主题 预设',exact:true});
 const darkPreset=page.getByRole('combobox',{name:'深色主题 预设',exact:true});
 assert.equal(await darkPreset.locator('option:not([disabled])').count(),6);
 for(const name of ['dsh','ocean','forest','violet','amber','rose']) {
   const untouched=await page.getByRole('textbox',{name:'浅色主题 背景色 HEX',exact:true}).inputValue();
   await darkPreset.selectOption(name);
   assert.equal(await page.getByRole('textbox',{name:'浅色主题 背景色 HEX',exact:true}).inputValue(),untouched);
   assert.equal(await darkPreset.inputValue(),name);
   await lightPreset.selectOption(name);
 }
 await page.screenshot({path:output+'/dsh-presets-rose.png'});
 await page.reload();
 await page.waitForFunction(()=>window.testCalls.some(c=>c.cmd==='plugin:event|listen' && c.args.event==='webchat-auth-hint'));
 await page.evaluate(()=>window.testReady());
 await page.locator('iframe').waitFor();
 ...
 assert.equal(await page.evaluate(()=>window.firstShellPaint.bg),'#2b1b23');
 assert.equal(await darkPreset.inputValue(),'rose');
 assert.equal(await lightPreset.inputValue(),'rose');
 await darkPreset.selectOption('dsh');
 await lightPreset.selectOption('dsh');
 const frameInput=page.getByRole('textbox',{name:'深色主题 边框色 HEX',exact:true});
 await frameInput.fill('#e8d7fa'); await frameInput.press('Tab');
 assert.equal(await page.locator('.titlebar').evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(232, 215, 250)');
 assert.equal(await page.locator('.ep-sidebar').evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(232, 215, 250)');
 assert.equal(await page.getByRole('textbox',{name:'深色主题 背景色 HEX',exact:true}).inputValue(),'#191919');
 assert.equal(await darkPreset.inputValue(),'custom');
 await page.screenshot({path:output+'/dsh-frame-color.png'});
 await page.reload();
 await page.waitForFunction(()=>window.testCalls.some(c=>c.cmd==='plugin:event|listen' && c.args.event==='webchat-auth-hint'));
 await page.evaluate(()=>window.testReady());
 await page.locator('iframe').waitFor();
 ...
 await page.locator('.tb-pill').click();
 await page.getByRole('button',{name:'外观',exact:true}).click();
 assert.equal(await frameInput.inputValue(),'#e8d7fa');
 await page.getByRole('article',{name:'深色主题',exact:true}).getByRole('button',{name:'重置主题',exact:true}).click();
 assert.equal(await darkPreset.inputValue(),'dsh');
 await page.getByRole('radio',{name:'浅色',exact:true}).click();
 const lightBackground=page.getByRole('textbox',{name:'浅色主题 背景色 HEX',exact:true});
 await lightBackground.fill('#fa0000'); await lightBackground.press('Tab');
 const surfaceInput=page.getByRole('textbox',{name:'浅色主题 前景色 HEX',exact:true});
 await surfaceInput.fill('#e8f5ff'); await surfaceInput.press('Tab');
 await page.waitForFunction(()=>getComputedStyle(document.querySelector('.ap-theme-card')).backgroundColor==='rgb(232, 245, 255)');
 assert.equal(await page.locator('.ap-theme-card').first().evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(232, 245, 255)');
 assert.equal(await lightBackground.inputValue(),'#fa0000');
 await page.screenshot({path:output+'/dsh-surface-red-background.png'});
 await page.getByRole('article',{name:'浅色主题',exact:true}).getByRole('button',{name:'重置主题',exact:true}).click();
 await page.getByRole('radio',{name:'深色',exact:true}).click();
 const lightCard=page.getByRole('article',{name:'浅色主题',exact:true});
 const darkCard=page.getByRole('article',{name:'深色主题',exact:true});
 await lightCard.getByRole('button',{name:'复制主题',exact:true}).click();
 await darkCard.getByRole('button',{name:'粘贴主题',exact:true}).click();
 await page.getByRole('status').filter({hasText:'主题已粘贴'}).waitFor();
 await page.waitForFunction(()=>document.querySelector('[aria-label="深色主题 背景色 HEX"]').value==='#fafafa');
 assert.equal(await page.getByRole('textbox',{name:'深色主题 背景色 HEX',exact:true}).inputValue(),'#fafafa');
 await page.evaluate(()=>window.copiedTheme='invalid JSON');
 await darkCard.getByRole('button',{name:'粘贴主题',exact:true}).click();
 await page.getByRole('alert').waitFor();
 assert.equal(await page.getByRole('textbox',{name:'深色主题 背景色 HEX',exact:true}).inputValue(),'#fafafa');
 await page.evaluate(()=>window.clipboardReadFail=true);
 await darkCard.getByRole('button',{name:'粘贴主题',exact:true}).click();
 assert.ok((await page.getByRole('alert').innerText()).includes('无法读取剪贴板'));
 await page.evaluate(()=>window.clipboardReadFail=false);
 await darkCard.getByRole('button',{name:'重置主题',exact:true}).click();
 ...
 // Low-contrast choices are user-owned, never rejected or auto-recolored.
 ...
 assert.equal(await foreground.inputValue(),'#fafafa');
 assert.equal(await page.getByRole('alert').count(),0);
 await foreground.fill('#242424'); await foreground.press('Enter');
